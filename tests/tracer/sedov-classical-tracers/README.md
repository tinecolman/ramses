# sedov-classical-tracers

<!-- BEGIN feature-coverage -->
## Feature areas covered by this test

Reaches 35 areas; most of its lines fall in **amr_core**, **refinement**, **particles**. 74 lines are reached by no other test in the suite.

| feature | area | lines | share of area | only this test |
|---|---|---:|---:|---:|
| amr_core | units | 10 | 100% |  |
| amr_core | shutdown | 61 | 90% |  |
| utilities | memory | 27 | 69% |  |
| particles | output | 85 | 69% |  |
| utilities | timer | 63 | 68% |  |
| amr_core | output | 236 | 65% |  |
| refinement | grid_linked_list | 445 | 65% |  |
| utilities | rng | 82 | 64% |  |
| amr_core | main_loop | 464 | 59% |  |
| utilities | neighbour_search | 113 | 59% |  |
| particles | particle_tree | 378 | 58% |  |
| hydrodynamics | output | 68 | 55% |  |
| utilities | file | 16 | 53% |  |
| utilities | communication | 303 | 53% |  |
| amr_core | dump_helpers | 20 | 50% |  |
| amr_core | grid_setup | 124 | 50% |  |
| hydrodynamics | hydro_core | 41 | 46% |  |
| refinement | flagging | 281 | 38% |  |
| hydrodynamics | setup | 222 | 30% |  |
| particles | particle_types | 11 | 28% |  |

15 further areas exercised more lightly (under 10 lines or 25% of the area, and shared with other tests).

<sub>Generated from `coverage_features_overview_2026-09-10_7b6d29b8` by `doc/features_overview/annotate_test_readmes.py`. *share* is of the area's executable lines; *only* counts lines no other test reaches.</sub>

<!-- END feature-coverage -->
