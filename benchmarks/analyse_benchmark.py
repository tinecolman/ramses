...
'''  '''
def plot_execution_time(benchmark_dir, reso_strong, nodes_strong):

    # gather data from log files
    times, errors_min, errors_max = [], [], []
    for nnodes in nodes_strong:
        #subdir_name = 'nodes'+str(nnodes)+'_reso'+str(reso_strong)
        subdir_name = 'nodes'+str(nnodes)+'_threads8'
        time, error_min, error_max = get_timings_total(benchmark_dir+'/'+subdir_name)
        times.append(time)
        errors_min.append(error_min)
        errors_max.append(error_max)
    asymmetric_error=np.array(list(zip(errors_min, errors_max))).T

    # make strong scaling plot
    plt.figure(figsize=[5,4])
    dates = ['today']
    for i in range(len(nodes_strong)):
        print(dates, [times[i]], [[errors_min[i]],[errors_max[i]]])
        plt.errorbar(dates, [times[i]], yerr=[[errors_min[i]],[errors_max[i]]], fmt='o', markersize=3, label=str(nodes_strong[i])+' nodes')
...
if __name__ == '__main__':
    benchmark_dir = '/home/tcolman/Dropbox/SPACE/benchmarks/meluxina/tests_openmp_3692b03d_sedov1024'
    reso_strong = 1024
    nodes_strong = [4,8,16,32,64]
    plot_execution_time(benchmark_dir, reso_strong, nodes_strong)